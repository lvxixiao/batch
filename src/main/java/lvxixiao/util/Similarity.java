...
public class Similarity {
    ...
    private double similarity(int age,boolean gender,double maxSumSim ,List<User> simUser){
        ...
        String sql = "select name,round(age) as age,gender,round(height) as height,round(weight) as weight from user " +
                "where round(age) = ? and gender = ? " +
                "order by round(height) desc,round(weight) desc";
    ...
    private Map<Double,List<User>> similarity(List<User> users){
        ...
        //从0开始，到size - 9 结束
        for(int i = 0 ; i < users.size() - 9 ; i++){
            double sumSim = 0;
            for(int j = i + 1; j < i + 10; j++){
                sumSim = sumSim + Cosine.simCosine(users.get(i),users.get(j));
            }
            //如果当前总相似度比记录的相似度大，则更改数据.
            if(maxSumSim < sumSim){
                maxSumSim = sumSim;
                list.clear();
                for(int z = i ; z < (i+10) ; z++){
                    list.add(users.get(z));
                }
            }
            /*
            //数据很难连续出现10次相等
            //总相似度大于等于9，说明10条数据相等，可以直接返回。
            //之所以会大于9，是因为double运算会有误差
            if(maxSumSim >= 9d){
                map.put(maxSumSim,list);
                return map;
            }
            */
