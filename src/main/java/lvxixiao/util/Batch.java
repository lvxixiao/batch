...
public class Batch {
  ...
  private int MaxNumber = 100000; // 最高一次可以执行批处理的数量
  ...
  public void batchInsert2(int number) {
    ...
    String insert = "insert into user(name, " + "gender, isMarried, age, height, weight, hobby, "
        + "address, disHistory, habit, target) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public void batchInsert(int number) {
    ...
    int time = 0;
    if(number > MaxNumber)
      time = number / MaxNumber; // 将number按照定义的数量拆分执行插入，避免堆溢出
    do{
    ...
    long endTime = System.currentTimeMillis();
    System.out.println(LocalDateTime.now() + ":插入" + number + "条数据耗时" + (endTime - startTime)
        + "ms");
  }
}
